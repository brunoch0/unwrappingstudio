    ...
    <div className="max-w-[760px]">
      ...
      <form action={saveProduct} className="mt-7 flex flex-col gap-5">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <Row name="collection_id" title="Collection">
          ...
          <Row name="sort" title="Sort order" hint="Lower shows first">
          ...
          <label className="flex items-center gap-2.5 pt-7" htmlFor="is_featured">
            ...
            <span className="text-[14px] text-[var(--text-body)]">Featured (curator&apos;s pick)</span>
          </label>
        </div>

        <Row name="curation_point" title="Curation point" hint="One-line tagline shown in quotes">
          <input id="curation_point" name="curation_point" defaultValue={product?.curation_point ?? ""} className={field} />
        </Row>
        <Row name="curation_comment" title="Why this piece" hint="Full curation comment">
          <textarea id="curation_comment" name="curation_comment" rows={4} defaultValue={product?.curation_comment ?? ""} className={`${field} resize-y`} />
        </Row>

        <div className="grid gap-5 sm:grid-cols-2">
          <Row name="material" title="Material"><input id="material" name="material" defaultValue={product?.material ?? ""} className={field} /></Row>
          <Row name="size" title="Size"><input id="size" name="size" defaultValue={product?.size ?? ""} className={field} /></Row>
          <Row name="care" title="Care"><input id="care" name="care" defaultValue={product?.care ?? ""} className={field} /></Row>
          <Row name="origin" title="Origin"><input id="origin" name="origin" defaultValue={product?.origin ?? ""} className={field} /></Row>
        </div>
        <Row name="includes" title="In the box"><input id="includes" name="includes" defaultValue={product?.includes ?? ""} className={field} /></Row>

        <Row name="thumbnail" title="Thumbnail URL"><input id="thumbnail" name="thumbnail" defaultValue={product?.thumbnail ?? ""} className={field} /></Row>
        <Row name="images" title="Gallery image URLs" hint="One per line (or comma-separated)">
          <textarea id="images" name="images" rows={3} defaultValue={(product?.images ?? []).join("\n")} className={`${field} resize-y`} />
        </Row>
        <Row name="badges" title="Badges" hint="Comma-separated, e.g. Ships to UAE, Limited run">
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Link href="/admin/products" className="us-btn us-btn--md us-btn--ghost">Cancel</Link>
        </div>
      </form>
